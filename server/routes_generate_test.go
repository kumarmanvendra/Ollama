...
type mockRunner struct {
	...
	CompletionFn func(context.Context, runners.CompletionRequest, func(runners.CompletionResponse)) error
}

func (m *mockRunner) Completion(ctx context.Context, r runners.CompletionRequest, fn func(r runners.CompletionResponse)) error {
...
`, createBinFile(t, fileutils.KV{
	...
	t.Run("messages with tools (non-streaming)", func(t *testing.T) {
		...
		mock.CompletionResponse = runners.CompletionResponse{
	...
	t.Run("messages with tools (streaming)", func(t *testing.T) {
		...
		mock.CompletionFn = func(ctx context.Context, r runners.CompletionRequest, fn func(r runners.CompletionResponse)) error {
			defer wg.Done()

			// Send chunks with small delays to simulate streaming
			responses := []runners.CompletionResponse{
				{
					Content:            `{"name":"get_`,
					Done:               false,
					PromptEvalCount:    1,
